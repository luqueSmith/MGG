-- =========================================================
--  1. LISTA DE DATOS (TEXTO CRUDO)
-- =========================================================
-- Aquí está tu lista original en minúsculas. 
-- NO IMPORTA que esté en minúsculas aquí, el script lo arregla abajo.
local raw_text = [[
...
-- Esta parte lee tu lista línea por línea y CONVIERTE A MAYÚSCULAS
for line in raw_text:gmatch("[^\r\n]+") do
    -- Busca el último texto después de un espacio (el código)
    local name, code = line:match("^(.-)%s+(%S+)$")
    
    if name and code then
        -- >>> AQUÍ ESTÁ LA CONVERSIÓN <<<
        -- string.upper() transforma "aa_99" en "AA_99" automáticamente
        code = string.upper(code) 
        
        table.insert(characters, {nombre = name, codigo = code})
    end
end
...
-- =========================================================
--  3. FUNCIONES DEL BUSCADOR (Interfaz)
-- =========================================================

local function showCharacterDetails(char)
    local msg = "PERSONAJE: " .. char.nombre .. "\nCÓDIGO: " .. char.codigo
    local choice = gg.alert(msg, "Copiar Código", "Cerrar", "Copiar Nombre")
    
    if choice == 1 then
        gg.copyText(char.codigo)
        gg.toast("Código copiado: " .. char.codigo)
    elseif choice == 3 then
        gg.copyText(char.nombre)
        gg.toast("Nombre copiado: " .. char.nombre)
    end
end

local function searchMenu()
    local input = gg.prompt({"Buscar personaje (Nombre):"}, nil, {"text"})
    ...
    for i, char in ipairs(characters) do
        if string.find(string.lower(char.nombre), query, 1, true) then
            table.insert(results, char)
        end
    end
    
    if #results == 0 then
        gg.alert("No encontrado: " .. input[1])
        return
    end

    local menuItems = {}
    for i, char in ipairs(results) do
        table.insert(menuItems, char.nombre .. " [" .. char.codigo .. "]")
    end
    
    local choice = gg.choice(menuItems, nil, "Resultados: " .. #results)
    if choice then
        showCharacterDetails(results[choice])
    end
end

local function viewAllMenu(page)
    page = page or 1
    local totalItems = #characters
    ...
    for i = startIndex, endIndex do
        -- Aquí ya se mostrará en MAYÚSCULAS porque lo convertimos al inicio
        table.insert(menuItems, characters[i].nombre .. " [" .. characters[i].codigo .. "]")
        table.insert(menuActions, {type = "char", data = characters[i]})
    end
    
    if page < totalPages then
        table.insert(menuItems, ">>> SIGUIENTE ("..(page+1)..") >>>")
        table.insert(menuActions, {type = "nav", page = page + 1})
    end
    if page > 1 then
        table.insert(menuItems, "<<< ANTERIOR ("..(page-1)..") <<<")
        table.insert(menuActions, {type = "nav", page = page - 1})
    end
    
    local choice = gg.choice(menuItems, nil, "Página " .. page .. " de " .. totalPages)
    
    if choice then
        local action = menuActions[choice]
        if action.type == "char" then
            showCharacterDetails(action.data)
            viewAllMenu(page)
        elseif action.type == "nav" then
            viewAllMenu(action.page)
        end
    end
end
...
function main()
    while true do
        ...
        local menu = gg.choice({
            "🔍 Buscar personaje",
            "📋 Ver lista completa (" .. #characters .. ")",
            "❌ Salir"
        }, nil, "Base de Datos")
        
        if not menu then return end
        
        if menu == 1 then searchMenu()
        elseif menu == 2 then viewAllMenu(1)
        elseif menu == 3 then os.exit()
        end
        gg.setVisible(true)
    end
end
