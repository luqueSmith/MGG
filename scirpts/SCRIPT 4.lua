...
function searchList(prompt)
    local input = gg.prompt({prompt}, nil, {"text"})
    ...
    if #matches == 0 then gg.alert("No encontrado.") return nil end
    local choice = gg.choice(menu, nil, "Selecciona:")
...
function replaceText(old_text, new_text)
    ...
    -- Hay que cargar los resultados en la memoria del script antes de editar
    local results = gg.getResults(count) 
    
    -- EDITAR TODO
    gg.editAll(":" .. new_text, gg.TYPE_BYTE)
    
    gg.toast("✅ ¡HECHO! " .. count .. " editados.")
    -- No limpiamos resultados al final para que puedas verlos si quieres
end
    ...
    while true do
        if gg.isVisible() then
            ...
            local choice = gg.choice({
                "1. Buscar por NOMBRE (Lista)",
                "2. Escribir CÓDIGO (Manual)",
                "3. Salir"
            }, nil, "Reemplazo Texto Exacto (:) Fixed")
            
            if choice == 1 then
                local old = searchList("¿Qué tienes?")
                if old then
                    local new = searchList("¿Qué quieres?")
